...
def test_undo_then_redo_moves_position_without_rerunning(monkeypatch):
    rendered = []
    monkeypatch.setattr(
        generation,
    ...
    # Stepping through history only moves the editor: the restored code is not
    # re-executed, so the user decides when to run it.
    assert rendered == []
